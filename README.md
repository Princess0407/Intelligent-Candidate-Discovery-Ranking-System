...
python precompute.py --candidates ./candidates.jsonl --base-dir .
...
|-------|--------|-----------|---------|
| **Offline** | `precompute.py` | BM25 indexing, weak label generation, LightGBM training | ~2 min |
| **1** | `retrieval.py` | Dual-Pass BM25 Retrieval (top 5,000 + rare-term pool) | 1–2s |
| **2** | `features.py` | 22-Feature Schema-Grounded Matrix extraction | 15–25s |
| **4** | `rank.py` | LightGBM LambdaRank inference | 1–3s |
| **5** | `reasoning.py` | Deterministic reasoning compilation | 1–2s |
| **6** | `rank.py` | Monotonicity assertion + diversity audit + CSV write | <1s |
...
The `app.py` Streamlit sandbox runs in **lite mode** (max 10,000 candidates, ≤1 GB RAM).
