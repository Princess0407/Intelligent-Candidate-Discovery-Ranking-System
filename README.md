...
│   ├── bm25_index.pkl              # Genuine fallback in src/rank.py if fast NumPy artifacts fail to load (146.2 MB)
│   ├── candidate_ids.pkl           # Core required index mapping BM25 rows to candidate IDs (1.5 MB)
│   ├── weak_labels.pkl             # Training label log from offline precomputation (not read during active inference) (2.4 MB)
│   └── lgbm_model.pkl              # Genuine fallback in src/rank.py if native text booster fails to load (1.4 MB)
